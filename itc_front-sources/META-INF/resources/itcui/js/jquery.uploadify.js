	...
	var methods = {
		...
		init : function(options, swfUploadOptions) {
			...
			return this.each(function() {
				...
				//IE11下无法检测flash是否安装
				if (flashInstalled || !!window.MSInputMethodContext) {
					...
					.css({
						'height'      : settings.height + 'px',
						'line-height' : (settings.lineheight || settings.height) + 'px',
						'width'       : settings.width + 'px'
					});
					...
					//链接的鼠标指向下划线效果 由于某种原因 这里没法用css完成
					$button.parents(".uploadify").mouseover(function(){
						$(this).find(".uploadify-button-text").css("text-decoration","underline");
					});
					$button.parents(".uploadify").mouseout(function(){
						$(this).find(".uploadify-button-text").css("text-decoration","none");
					});
	...
	$.fn.extend({
		ITCUI_FileUpload : function(options,arg){
			...
			_t.init = function(){
				...
				//2017.1.16 移动到最前
				$("#" + queueId).on("click", ".itc_upload_top", function(e){
					var target = $(e.target);
				...
				//目前只允许上传一个文件 或者无限
				options.single = options.single || options.singleFile || false;
				options.width = options.width || "72";
				options.itemTemplate = options.itemTemplate || '<div class="itc_upload_wrap bbox ${preview}" fileID="${fileID}">' + 

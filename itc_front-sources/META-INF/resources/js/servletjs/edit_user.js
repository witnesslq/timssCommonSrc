var form = [];
var privTmpl = template.compile("<ul>" +
	"{{each origins as item}}" +
	"<li>({{if item.type=='role'}}角色{{else}}用户组{{/if}})<span>{{item.name}}</span></li>" +
	"{{/each}}" +
	"</ul>");
$(document).ready(function(){
	...
	form= [
		{
			...
			rules : {
				...
				maxlength : 20,
				remote : {
		            "url": basePath + "userConfig?method=exist",
					type: "post",
					data: {
						uid: function() {
							return $("#f_uid").val();
	...
	if(mode!="create"){
		...
		showEditBtn();
		$("#f_orgs").html("请点击上方“修改公司/部门”选择，没有组织的用户无法在组织机构树中显示");
	}
...
function initPrivArea(){
	$("#site_priv").iFold("init");
	$("#priv_tree").tree({
		url : basePath + "tree?method=userpriv&uid=" + g.uid,
		onSelect: function(node){
			var origins = node.origins;
			var s = "";
			if(origins && origins.length){
				s += "<p>该权限从以下角色或用户组获得：</p>"
				s+= privTmpl(node);
			}
			$("#priv_wrap").html(s);
		}
	...
	$("#btnFold").on("click", function(){
		$("#priv_tree").tree("collapseAll");
	});
	$("#btnExpand").on("click", function(){
		$("#priv_tree").tree("expandAll");
	});
}
...
function save(){
	...
	$.ajax({
		url : basePath + "userConfig?method=" + mode,
		data : f,
		type : "post",
		dataType : "json",
...
function delUser(){
	Notice.confirm("确认删除|是否要删除用户“" + g.name + "”？该操作无法撤销。",function(){
		$.ajax({
			url : basePath + "userConfig?method=deluser&uid=" + g.uid,
			dataType : "json",
			type : "post",
			success : function(data){
				if(data.status==1){
...
function setUserStat(){
	...
	$.ajax({
		url : basePath + "userConfig?method=setstat&ids=" + g.uid + "&stat=" + pstat,
		dataType : "json",
		success : function(data){
			if(data.status==1){
				g.status = stat;
				...
				$("#form_baseinfo").iForm("setVal",{status: newVal});
			}
		}
	});
}

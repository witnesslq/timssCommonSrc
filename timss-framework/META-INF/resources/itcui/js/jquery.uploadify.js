...
(function($) {
	...
	var handlers = {
		...
		onDialogOpen : function() {
			...
			this.queueData.errorMsg       = '某些文件无法添加到队列中:';
		...
		onSelect : function(file) {
			...
			var fileSize = _itc_getfilesize(file.size);
				...
				switch(errorCode) {
					case SWFUpload.QUEUE_ERROR.QUEUE_LIMIT_EXCEEDED:
						if (settings.queueSizeLimit > errorMsg) {
							...
							this.queueData.errorMsg += '\n已选择的文件超出了单次上传的总大小限制 (' + settings.queueSizeLimit + ')。';
						}
						break;
					...
					case SWFUpload.QUEUE_ERROR.ZERO_BYTE_FILE:
						...
						break;
					case SWFUpload.QUEUE_ERROR.FILE_EXCEEDS_SIZE_LIMIT:
						this.queueData.errorMsg += '\n文件 "' + file.name + '" 类型错误 (只能上传扩展名为' + settings.fileTypeDesc + '的文件)。';
						break;
				}
			}
	...
	$.fn.extend({
		ITCUI_FileUpload : function(options,arg){
			...
			_t.init = function(){
				...
				/*##2014-07-09 beta7改为自动上传 取消开始上传按钮*/
				/*
				...
				*/
				...
				options.buttonText = options.buttonText || "添加附件";
				options.buttonClass = options.buttonClass || "itc_link_add";
				options.auto = true;
				options.height = 24;
				...
				options.itemTemplate = options.itemTemplate || '<div class="itc_upload_wrap bbox" fileID="${fileID}">' + 
					...
					'<div class="itc_upload_detail_wrap ml4">' +
						'<div class="itc_upload_filename">' +
							'<span class="itc_filename">${fileName}</span>' +
						'</div>' +
						'<div style="clear:both" class="itc_upload_progress_line">' +
							'<div class="itc_upload_progress_txt" style="margin-left:-4px">' +
								...
								'<a onclick="$(\'#${instanceID}\').uploadify(\'cancel\',\'${fileID}\')" class="itc_upload_cancel itc_link fr">取消</a>' +
								'<a class="itc_upload_delfail itc_link_del itc_link fr" style="display:none">删除</a>' +
				...
				options.onUploadSuccess = options.onUploadSuccess || function(file, data, response){
					...
					var _this = $(this);
					var q = $("#" + _this[0].settings.queueID);
					var options = q.data("options");
					var fileWrap = q.children("[fileID='" + file.id + "']");
					...
					if(data.status==1){
						...
						fileWrap.find(".itc_upload_cancel").remove();
						if(data.msg){
							//这里需要把msg存到每个框框里 要不多文件的时候id会乱掉
							fileWrap.data("ajaxMsg",data.msg);
							if(options.downloadFileUrl || options.delFileUrl){
								fileWrap.attr("fileID",data.msg);
							}
							var delLink = fileWrap.find(".itc_upload_delfail");
							fileWrap.find(".itc_upload_delfail").css('display','block').each(function(){
								$(this).removeClass("itc_upload_delfail");//防止重复绑定
								_itc_binddel(this);
							});
						}
						/*##beta7：对于新上传的文件没有下载按钮*/
						/*
						if(options.downloadFileUrl){
							fileWrap.find(".itc_upload_down").css('display','block').each(function(){
								$(this).removeClass("itc_upload_down");//防止重复绑定
				...
				options.onUploadStart = function(file){
					var _this = $(this);
					_this.data("isUploadFinish",false);
				...
				options.onQueueComplete = function(){
					$(this).data("isUploadFinish",true);
				}

				$("#" + realId).uploadify(options);	
				var q = $("#" + options.queueID);
				q.addClass("upload-queue").data("options",options);
				//显示已经上传的文件
			...
			_t.isUploadFinish = function(){
				var realId = $(this).attr("id") + "_real";
				return $("#" + realId).data("isUploadFinish");
			};
			...
			_t.createInitList = function(initFiles){
				...
				for(var i=0;i<initFiles.length;i++){
					...
					blk = blk.replace(new RegExp("\\$\{fileName\}"),fnStr);
					blk = blk.replace(new RegExp("\\$\{fileExt\}"),_itc_getextname(file.fileName));
					blk = blk.replace(new RegExp("\\$\{fileSize\}"),_itc_getfilesize(file.fileSize));
					fileHtml += blk;
				}
				q.children("[init=true]").remove();
				q.append(fileHtml);
				q.find(".upload-viewdoc").click(function(){
					var _this = $(this);
					var id = _this.attr("id");
					var ext = _this.attr("ext");
					if(/(doc|docx|xls|xlsx)/.test(ext)){
						var activeX = document.getElementById("docViewCtrl");
						var fileWrap = _this.parents(".itc_upload_wrap");
						var msgWrap = _this.parents(".itc_upload_wrap").find(".itc_upload_progress_txt").children("span");
						msgWrap.html("正在准备文件并打开");
						
						var opts = fileWrap.parent().data("options");
						if(opts.tempKeyUrl){
							$.ajax({
								url: opts.tempKeyUrl,
								data : {
								...
								dataType : "JSON",
								success : function(r){
									var url = opts.downloadFileUrl + "&key=" + r.msg + "&ext=." + ext
									var result = activeX.open(url);
		       						if(result!="success"){
		       							msgWrap.html(result);
		       						}
		       						else{
				...
				if(options.delFileUrl){
					q.find(".itc_upload_delfail").each(function(){
						$(this).parents(".itc_upload_wrap").attr("init",true);
						$(this).removeClass("itc_upload_delfail").show();
						_itc_binddel(this);
					});
				}
				if(options.downloadFileUrl){

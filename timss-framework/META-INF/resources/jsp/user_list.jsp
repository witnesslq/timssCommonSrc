...
<script>
	...
	function reloadDatagrid(){	
		var columns = [
	    	...
	    	{field:'mail',width:160,title:'电子邮箱',fixed:true},
	    	{field:'type',width:70,title:'账户类型',fixed:true},
	    	{field:'status',width:50,title:'状态',fixed:true},
	    	{field:'nouse',width:60,title:''}
	    ];
	    ...
	    tbUser = $("#table_user").datagrid({
			fitColumns:true,
	        url:"${basePath}user?method=getusers",
	        queryParams:{
	        	"filter":filter,
			...
			onDblClickRow : function(rowIndex, rowData) {
				window.location.href = "${basePath}user?method=getuser&mode=edit&uid=" + rowData.uid;
			}
		});
	}
